...
macro_data=([FEDGOV.FEDGOV(p_factor+1:end),N.N(p_factor+1:end), GDP.GDP(p_factor+1:end),CONS.CONS(p_factor+1:end),SUR.SUR(p_factor+1:end),FED_FUNDS.FED_FUNDS(p_factor+1:end),CP.CP(p_factor+1:end),C_SD_LNCONS_SA.C_SD_LNCONS_SA(p_factor+1:end)]); 
VARnames={'Government Spending';'News Variable';'Real GDP';'Consumption';'Federal Surplus';'Fed Funds Rate';'Corporate Profits';'Consumption Inequality'};

% macro_orthogonality = table2array([FEDGOV,N,GDP,BONDY,SUR,RER,SP500,FED_FUNDS]); 
% opt.q=size(macro_orthogonality,2); %number of variables excluding the factors
% [opt.f,mdl_suff_surprise,mdl_ortho_surprise,mdl_suff_news,mdl_ortho_news,mdl_surprise,mdl_news,mdl_surprise_ortho,mdl_news_ortho] = check_orthogonality(macro_orthogonality,factor,opt);
