function [ehat,Fhat,lamhat,ve2,x2] = factors_em(x,r,jj,DEMEAN)
% =========================================================================
% DESCRIPTION
...
%           r    =   an integer indicating the number of factor to be estimated
...
% Check that r is an integer between 1 and the number of columns of x,
% or 99
if ~((r<=size(x,2) && r>=1 && floor(r)==r) || r==99)
    error('Input r is specified incorrectly.');
end
...
% If input 'r' is not set to 99, use subfunction baing() to determine
...
while err> 0.000001 && it <maxit
    ...
    % subfunction baing() (or set to 8 if r equals 99)
    if r ~=99
        [icstar,~,~,~]=baing(x3,r,jj);
    else
        icstar=8;
    end
...
function [ic1,chat,Fhat,eigval]=baing(X,r,jj)
% =========================================================================
% DESCRIPTION
...
% INPUTS
%           X       = dataset (one series per column)
%           r    = an integer indicating the maximum number of factors
%                     to be estimated
%           jj      = an integer indicating the information criterion used 
%                     for selecting the number of factors; it can take on 
%                     the following values:
...
%         factors to be selected (from 1 to r).
%
...
% Preallocate memory
Sigma=zeros(1,r+1); % sum of squared residuals divided by NT
IC1=zeros(size(CT,1),r+1); % information criterion value
...
% Factors and loadings when number of factors set to r
Fhat=Fhat0(:,1:r); % factors
Lambda=Lambda0(:,1:r); % factor loadings

% Predict X using r factors
chat=Fhat*Lambda';
