...
%  Small-scale VAR: G, GDP, Surplus, Bond, C_SD (no Ft(1,4))
...
%  Full FAVAR (for IRF): [G, Ft(1,4), GDP, Surplus, Bond, C_SD, PC1-5] = 11 vars
...
favar_orth = [FEDGOV.FEDGOV, GDP.GDP, SUR.SUR, BONDY.x10YBOND, ...
              C_SD_LNCONS_SA.C_SD_LNCONS_SA, factor(:, 1:5)];
opt_favar      = opt;
opt_favar.q    = size(favar_orth, 2);   % = 10
pval_surp_E5 = check_orthogonality(favar_orth, factor, opt_favar);

save_sufficiency_table(pval_surp_E5, size(factor, 2), 'E.5', fullfile(tab_dir, 'TableE5.txt'));
